...
public class ComedyServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            // Must perfectly match the value string in your SQL database seed block: 'Comedy'
            List<ProductModel> books = new ProductDAO().getProductsByGenre("Comedy");
            request.setAttribute("genreBooks", books);
            request.getRequestDispatcher("/WEB-INF/pages/comedy.jsp").forward(request, response);
        } catch (Exception e) {
            e.printStackTrace();
